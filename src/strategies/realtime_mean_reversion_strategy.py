...
class RealtimeMeanReversionStrategy(RealtimeStrategy):
    def __init__(
        ...
        super().__init__("RealtimeMeanReversion", symbols, parameters)
    ...
    def on_tick(self, symbol: str, quote: Dict) -> Optional[TradingSignal]:
        ...
        if len(self.price_history[symbol]) > self.lookback_period + 10:
            self.price_history[symbol] = self.price_history[symbol][-(self.lookback_period + 10):]
        ...
        if not self.position_open[symbol] and z_score <= -self.entry_threshold:
            ...
            if quantity > 0:
                ...
                return TradingSignal(
                    ...
                    price=price,
                    quantity=quantity,
                    reason=f"均值回归买入：Z分数 {z_score:.2f}（超卖）",
                    confidence=min(abs(z_score) / self.entry_threshold * 0.8, 1.0)
                )

        if self.position_open[symbol] and abs(z_score) <= self.exit_threshold:
            ...
            return TradingSignal(
                ...
                price=price,
                quantity=0,
                reason=f"均值回归卖出：Z分数 {z_score:.2f}（回归均值）",
                confidence=min(abs(z_score) / self.entry_threshold * 0.8, 1.0)
            )
